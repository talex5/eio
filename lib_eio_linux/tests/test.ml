...
let test_direct_copy () =
  ...
  Switch.run (fun sw ->
      Fiber.fork ~sw (fun () -> Tracing.label "copy1"; Eio.Flow.copy from_pipe1 to_pipe2; Eio.Flow.close to_pipe2);
      Fiber.fork ~sw (fun () -> Tracing.label "copy2"; Eio.Flow.copy from_pipe2 to_output);
