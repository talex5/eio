...
(* todo: dispatching a separate job for every entry is slow; batch them *)
let next_dir_entry h () =
  in_worker_thread "with_dir_entries" @@ fun () -> read_entry h

let with_dir_entries dirfd path fn =
  let h =
    in_worker_thread "readdir" @@ fun () ->
    open_dir_handle "readdir" dirfd path
  in
  match fn @@ Seq.of_dispenser @@ next_dir_entry h with
  | v -> Unix.closedir h; v
  | exception ex ->
    let bt = Printexc.get_raw_backtrace () in
    Unix.closedir h;
